...
const getInitialData = () => {
  ...
  const defaultUsers: User[] = [
    { id: 'user-admin', name: 'Admin User', roleId: '17936e38-8e08-489d-ac5c-2c5ea22a0f76', email: 'admin@clinic.com' },
...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedState) {
        ...
        setRbacData({
          roles: parsedState.roles || initialData.roles,
          users: parsedState.users || initialData.users,
        });
  ...
  const handleLoginAttempt = async (username: string, pass: string): Promise<User | null> => {
    console.log("login users--", users, username, pass);
